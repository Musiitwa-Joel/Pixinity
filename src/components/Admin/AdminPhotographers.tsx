...
const AdminPhotographers: React.FC = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "Failed to fetch photographer stats:",
          await response.text()
        );
      }
    } catch (error) {
  ...
  const fetchPhotographers = async () => {
    ...
    try {
      ...
      console.log(
        "Fetching photographers with params:",
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch photographers: ${response.status} ${response.statusText}`
        );
      }

      const data = await response.json();
      console.log(`Fetched ${data.photographers.length} photographers`);
  ...
  const handleToggleVerified = async (photographer: Photographer) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/admin/users/${photographer.id}/toggle-verification`,
        {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      setPhotographers(
        photographers.map((p) =>
          p.id === photographer.id ? { ...p, verified: data.verified } : p
        )
      );
  ...
  const handleDeletePhotographer = async (photographerId: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/admin/users/${photographerId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          ...
          },
          {
            ...
            value: stats.verifiedPhotographers,
            icon: CheckCircle,
          ...
          },
          {
            ...
            value: stats.featuredPhotographers,
            icon: Star,
